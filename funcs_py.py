...
def calculate_deviations(xsiz, zsiz, pix2nm, dt, u, g, b, c2d, nu, phi):
    ...
    for x in range (xsiz):
        for z in range(zsiz+1):
            ...
            rD = np.array((0.5+x-xsiz/2, (0.5+z-zsiz/2)*dt*np.sin(phi), 0))#in pixels
            rD = rD*pix2nm#in nm
            #Displacement R is calculated in the crystal frame
            gR = gdotR( rD, bscrew, bedge, beUnit, bxu, d2c, nu, g )
            gRdz = gdotR( rD+deltaz, bscrew, bedge, beUnit, bxu, d2c, nu, g )
            sxz[x,z] = (gRdz - gR)/dz
...
def calculate_image(sxz, xsiz, ysiz, zsiz, pix2nm, t, dt, phi, s, Xg, X0i, g, b):
    ...
    for x in range(xsiz):
        for y in range(ysiz):
            F = F0[:]
            # z loop propagates the wave over a z-line of sxz of length n 
            # corresponding to the thickness of the foil (t/dt slices)
            zlen=int(t/dt + 0.5)
            # For this row the top of the foil is
            top = zsiz-zlen-y/(dt*np.tan(phi)) # in pixels
            h=int(top)
