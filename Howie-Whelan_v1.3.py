...
#point the dislocation passes through - the centre of simulation volume
p = np.array((pad+0.5, 0.5, (hpad+zmax)*dt+0.5)) 
...
for i in range (xmax):
    ...
    for k in range(zrange):
        ...
        v = np.array((i, 0, k*dt))
        xyzS = pix2nm*(v-p)#in nm
        xyz = s2c @ xyzS
        R = displaceR( xyz, b, u, c2d, d2c )
        gdotR = np.dot(g,R)
        xyzSdz = pix2nm*(v-p+dz)
        xyzdz = s2c @ xyzSdz
...
##################################    
    ...
    imgname="BF_t="+str(int(t))+"_s"+str(s_int)+".tif"
    spoink = tif.open(imgname, mode='w')
    spoink.write_image(Ib)
    spoink.close
    imgname="DF_t="+str(int(t))+"_s"+str(s_int)+".tif"
    spoink = tif.open(imgname, mode='w')
    ...
    bbox_inches=0
    plotnameP="t="+str(int(t))+"_s"+str(s_int)+".png"
    print(plotnameP)
