...
__kernel void displacement(__global float *image_out,
                           ...
                           float b_screw, float dt, float dz, float nu, float phi) {
    ...
    float x = ((float) xid + 0.5f - (float) xsiz / 2)*pix2nm;//+ 0.5f
    float y = ((float) zid + 0.5f + dz - (float) zsiz / 2)*dt*sin(phi)*pix2nm;//
    float3 r_d = {x, y, 0.0f};
...
__kernel void propagate_wave(__global float4 *in_out, __global float *sxz,
    ...
    int yid = get_global_id(1);
    int xsiz = get_global_size(1);
    int ysiz = get_global_size(0);
    // position of the current pixel in the linear array
    int id = xid * xsiz + yid;

    //top of the foil for this pixel
    float top = zsiz - yid / (dt*tan(phi)) -zlen;
    int h = (int) top;
    float m = top - h;
    
    int id_2 = (h + k) * ysiz + xid;
    int id_3 = (h + k + 1) * ysiz + xid;
...
class ClHowieWhelan:
    ...
    def calculate_deviations(self, xsiz, zsiz, pix2nm, dt, u, g, b, c2d, d2c, nu, phi):
        #fills up sxz array
        ...
        self.displace_r(shape, self.sxz_buf, c2d_buf, d2c_buf, pix2nm, u, g, b, c2d, nu, phi, dt, dz)
        # calculate second array at a small z-shift
        dz = 0.01
        self.displace_r(shape, out_buf_2, c2d_buf, d2c_buf, pix2nm, u, g, b, c2d, nu, phi, dt, dz)
    ...
    def displace_r(self, shape, out_buf, c2d_buf, d2c_buf, pix2nm, u, g, b, c2d, nu, phi, dt, dz):
    ...
    def calculate_image(self, xsiz, ysiz, zsiz, pix2nm, t, dt, s, Xg, X0i, g, b, phi):
        ...
        zlen_32 = np.int32(t/dt + 0.5)
        for k in range(zlen_32):
            k_i32 = np.int32(k)

            self.image_prog.propagate_wave(self.queue, shape, None, in_out_buf, self.sxz_buf,
                                        g_32, b_32, zsiz_i32, t_32, dt_32, pix2nm_32, 
                                        zlen_32, s_32, x_g_32, x_0_i_32, phi_32, k_i32)
